...
class ann:
	...
	TODO: variable size of hidden layers
	...
	'''
	def __init__(self, n_features=1, classification_set=None, n_hidden_layers=1, hidden_layers_size=5, activation='sigmoid'):
		assert (activation in HIDDEN_ACTIVATIONS)
		assert isinstance(n_features, int) and n_features > 0
		assert isinstance(n_hidden_layers, int) and n_hidden_layers >= 0 #allow for no hidden layer
		assert isinstance(hidden_layers_size, int) and hidden_layers_size > 0
		...
		if classification_set is None:
			...
			self.cls2idx = dict(zip(classification_set, xrange(self.n_outputs))) # {[cls:i]}
			self.idx2cls = dict(enumerate(classification_set)) # {[i:cls]}
		self.hidden_layers_size = hidden_layers_size
		self.n_hidden_layers = n_hidden_layers
		self.n_layers = n_hidden_layers + 2
		self.activation = activation
		self.layers_size = [self.n_features] + [self.hidden_layers_size] * self.n_hidden_layers + [self.n_outputs]
		self.n_param = sum([(self.layers_size[i]+1)*self.layers_size[i+1] for i in xrange(self.n_hidden_layers + 1)])
		# Initialise the parameters
		self.W = [];
		self.b = [];
		for i in xrange(self.n_hidden_layers + 1):
			self.W.append(np.random.randn(self.layers_size[i], self.layers_size[i + 1])) #/ sqrt(self.n_param))
			self.b.append(np.random.randn(self.layers_size[i + 1])) #/ sqrt(self.n_param))
	...
	def _forward(self, Z):
		...
		Returns
		_______
		list, length = self.n_hidden_layers + 2
	...
	def fit(self, X, Y, Xtest=None, Ytest=None, learning_rate=0.1, reg_rate=0., epochs=5000, verbose=False, n_prints=100,
		...
		if self.n_hidden_layers == 0:
			...
			if self.n_hidden_layers > 1:
				msg += str(self.n_hidden_layers) + " hidden layers of "
			msg += str(self.hidden_layers_size) + " hidden neurons and " + self.activation + " activation "
		...
		for epoch in xrange(epochs):
			...
			for i in xrange(self.n_hidden_layers, -1, -1):
