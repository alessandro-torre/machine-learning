...
class ann:
    ...
    """
    def __init__(self, n_features=1, classification_set=None, hidden_layers_shape=0, batch_size=None,
                 activation='sigmoid'):
        ...
        self._init_params()
        assert (isinstance(batch_size, int) and batch_size > 0) | (batch_size is None)
    ...
    def fit(self, X, Y, Xtest=None, Ytest=None, refit=False, learning_rate=0.1, reg_rate=0., epochs=5000, batch_size=None, verbose=False, n_prints=100,
        ...
        assert (isinstance(batch_size, int) and batch_size > 0) | (batch_size == None)
        ...
        batch_size = self.batch_size if batch_size == None else batch_size # Use value set up at init
        batch_size = min(batch_size, X.shape[0]) if batch_size != None else X.shape[0]
        ...
        for epoch in range(epochs):
            ...
            for b in range(n_batches):
                ...
                x = Xtrain[b*batch_size: (b+1)*batch_size, :]
                t = Ttrain[b*batch_size: (b+1)*batch_size, :]
                z = self._forward(x)
